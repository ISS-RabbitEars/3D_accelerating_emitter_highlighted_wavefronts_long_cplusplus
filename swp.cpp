...
int main()
{
    ...
    double xa,ya,za,sigma,k,omega,phi,lambda,nlambda,delt,d,a,alpha,beta,lt,rt,vs;
    ...
    vs=omega/k;
    d=(lt+rt)*xa;
    a=((beta*beta-alpha*alpha)*vs*vs)/(2.*d);
    delt=(beta-alpha)*vs/(a*(nframes-1));
    /*------------------------*/
    ...
    {
        ti[i]=i*(nframes/Ne);
        rc[i]((-lt*xa)+(alpha*vs*ti[i]*delt)+(0.5*a*ti[i]*ti[i]*delt*delt));
    }
